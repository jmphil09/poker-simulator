...
object multipleHands {
  ...
  def compareMultHands(handList: List[(Hand, Int)]): List[List[String]] = {
    ...
    def createStrList(orderedHands: List[(Int, Hand, Int)], result: List[List[String]]): List[List[String]] = {
      ...
      def genList(listNums: List[Int], players: List[String]): List[String] = {
        if (listNums.isEmpty) players
        else {
          val playerStr = "Player " ++ listNums.head.toString
          genList(listNums.tail, players ++ List(playerStr))
        }
      }
      ...
      if (orderedHands.isEmpty) result
        ...
        createStrList(subtractPlayers(bestPlayers, orderedHands), result ++ List(genList(bestPlayers, List())))
      }
    }
